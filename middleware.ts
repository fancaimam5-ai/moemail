...
import { i18n, type Locale, LEGACY_LOCALES } from "@/i18n/config"
import { PERMISSIONS } from "@/lib/permissions"
import { checkPermission } from "@/lib/auth"
import { Permission } from "@/lib/permissions"
import { handleApiKeyAuth } from "@/lib/apiKey"
import { isUserSuspended } from "@/lib/admin"
...
export async function middleware(request: Request) {
  ...
  if (pathname.startsWith('/api')) {
    ...
    if (
      ...
      pathname.startsWith('/api/emails/guest-generate') ||
    ...
    // Admin routes handle their own EMPEROR-level auth internally
    ...
    // Check if user is suspended (skip for admin routes - admin handles its own auth)
    if (!pathname.startsWith('/api/admin') && session.user.id) {
      const suspended = await isUserSuspended(session.user.id)
      if (suspended) {
        return NextResponse.json(
  ...
  // Redirect legacy locales (zh-CN, zh-TW, ja, ko) to English
  if (LEGACY_LOCALES.includes(maybeLocale as any)) {
    segments[1] = i18n.defaultLocale
    const redirectURL = new URL(segments.join('/') + url.search, request.url)
