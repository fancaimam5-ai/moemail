...
export default getRequestConfig(async ({locale}) => {
  ...
  try {
    ...
    const pricing = (await import(`@/i18n/messages/${safeLocale}/pricing.json`)).default
    const admin = (await import(`@/i18n/messages/${safeLocale}/admin.json`)).default
    const pages = (await import(`@/i18n/messages/${safeLocale}/pages.json`)).default
    const checkout = (await import(`@/i18n/messages/${safeLocale}/checkout.json`)).default
    return {locale: safeLocale, messages: {common, home, auth, metadata, emails, profile, pricing, admin, pages, checkout}}
  } catch {
    return {locale: safeLocale, messages: {common: {}, home: {}, auth: {}, metadata: {}, emails: {}, profile: {}, pricing: {}, admin: {}, pages: {}, checkout: {}}}
  }
})
