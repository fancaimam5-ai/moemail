...
import { Loader2, Send, ChevronDown, ChevronUp, Copy } from "lucide-react"
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
export function WebhookConfig() {
  ...
  useEffect(() => {
    fetch("/api/webhook")
      .then(res => res.json() as Promise<{ enabled: boolean; url: string; signingSecret: string }>)
      .then(data => {
  ...
  const handleCopySecret = () => {
    ...
    navigator.clipboard.writeText(signingSecret).then(() => {
      toast({ title: tApiKey("copy") })
    })
  }
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button type="button" variant="outline" onClick={handleCopySecret} className="flex-shrink-0">
              ...
              <p className="text-xs text-muted-foreground">{t("signingSecretDescription")}</p>
            </div>
          )}
