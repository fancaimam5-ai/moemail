...
import { Github, Settings, Crown, Sword, User2, Gem, Mail, Package, Sparkles, Trash2, Receipt } from "lucide-react"
...
export function ProfileCard({ user }: ProfileCardProps) {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      toast({ title: tDelete("success") })
      await signOut({ callbackUrl: `/${locale}` })
    } catch {
      toast({ title: tDelete("failed"), variant: "destructive" })
    } finally {
      setIsDeleting(false)
    }
  }

  const [quota, setQuota] = useState<{ totalCreated: number; max: number; planName: string; isPremium: boolean } | null>(null)
  const [premiumPrice, setPremiumPrice] = useState<string | null>(null)

  useEffect(() => {
    fetch("/api/emails/quota")
      .then(r => r.json())
      .then((data) => setQuota(data as { totalCreated: number; max: number; planName: string; isPremium: boolean }))
      .catch(() => {})

    fetch("/api/plans")
      .then(r => r.json())
      .then((data: any) => {
        const plan = data?.premium?.idr ?? data?.premium?.usd ?? null
        if (plan?.priceCents) {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-3 flex-wrap">
              <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                ...
                {quota.isPremium && <span className="mr-1">✨</span>}
                {quota.isPremium ? t("planPremium") : t("planFree")}
              </span>
              <span className="text-sm text-muted-foreground">
                {quota.max === -1
                  ? t("planUsageUnlimited", { used: quota.totalCreated })
                  : t("planUsage", { used: quota.totalCreated, max: quota.max })
                }
              </span>
            </div>
            {!quota.isPremium && (
              ...
              >
                ...
                {t("upgrade")}
                {premiumPrice && <span className="font-bold">{premiumPrice}/{t("perMonth")}</span>}
              </Button>
            )}
          </div>
          {!quota.isPremium && (
            <div className="mt-3 text-xs text-muted-foreground space-y-0.5">
              <p>{t("planFreeFeatures")}</p>
              <p className="text-amber-600 dark:text-amber-400">{t("planPremiumFeatures")}</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-3 px-1">
        ...
        <Button
          variant="secondary"
          onClick={() => router.push(`/${locale}/moe/payments`)}
          className="gap-2 flex-1"
        >
