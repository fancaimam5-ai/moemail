...
import { sendMagicLinkEmail } from "@/lib/email/tokens"
...
export async function POST(request: Request) {
  try {
    ...
    const result = await sendMagicLinkEmail(email.toLowerCase(), ipHeader)
    ...
    return NextResponse.json({ success: true, message: "If an account with this email exists, a magic link has been sent." })
  } catch {
