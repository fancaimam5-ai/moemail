      ...
      try {
        ...
        if (webhook.signingSecret) {
          const key = await crypto.subtle.importKey(
            "raw",
            new TextEncoder().encode(webhook.signingSecret),
            { name: "HMAC", hash: "SHA-256" },
            false,
            ["sign"]
          )
          const sig = await crypto.subtle.sign("HMAC", key, new TextEncoder().encode(webhookBody))
          headers["X-Webhook-Signature"] = `sha256=${Array.from(new Uint8Array(sig)).map((b: number) => b.toString(16).padStart(2, "0")).join("")}`
        }

        await fetch(webhook.url, { method: 'POST', headers, body: webhookBody })
      } catch (error) {
